...
class Logger extends _Logger {
  ...
  _log<T>(
    level: number,
    msg:
      | (T extends GenericFunction ? never : T)
      | (() => T),
    ...args: T[]
  ): T | undefined {
    // @ts-ignore
...
export const flushLogs = fileHandler.flush.bind(
  fileHandler,
);
...
const handlersDefault = {
  ...
  fileRotating: new handlers.RotatingFileHandler(
    "DEBUG",
    {
      maxBytes: 1024 * 10,
      maxBackupCount: 10,
